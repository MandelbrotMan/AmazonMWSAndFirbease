...
const amzKeys = require('./Config/dev');
...
var options = {
    ...
    apiKey: firebaseConfig.apiKey, // for Mapquest, OpenCage, Google Premier
...
var mws = require('amazon-mws-node')({
        AmzSecretKey: amzKeys.AmazonSecretKey,
        AWSAccessKeyId: amzKeys.AmazonClientSecretID
        });
...
function findRepeatingCustomer(OrdersInCity){
    //This only works for a sorted list of sales by city
    for(let i = 1; i < ordersInCity.length; ++ i){
        ...
        if(address1.toLowerCase() === address2.toLowerCase() && address1 !== null)
            // We want to know the most recent date of the order
...
function getCitiesCount(){
        //COOITC is count of orders in this city
...
//Old function for getting the gelocation for a list of orders. Now the getting the geolocation is performed 
//right away so this is no longer needed
...
//Function for filtering out sales that have been shipped. Unshipped orders will 
//do not contain certain properties
function processSalesFromMWS(jsObject){
    ...
    for(i = 0; i < total; ++i){
        if(salesFromMWS[i].OrderStatus == "Shipped"){  
            ...
            //Properties that maybe missing for some orders
            try{
...
//Bulk update to firebase. This path is all inclusive 
...
//Bulk update to firebase. This path is for the city.
...
/////////////BULKY OPERATIONS No longer used   
